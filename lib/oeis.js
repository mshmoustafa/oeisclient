...
var OEIS = /** @class */ (function () {
     ...
     */
    function OEIS(language, useProxy) {
        /**
     ...
     */
    OEIS.prototype.searchByID = function (id, format, callback) {
        if (!format) {
            format = "html";
        }
        var URL = this.endpoint + "search?" + "language=" + this.language + "&" + "q=" + "id:" + id + "&" + "fmt=" + format;
     ...
     * @param context an optional context in which to call the callback (callback.apply(context)).
     */
    OEIS.prototype.searchByTerms = function (terms, format, callback) {
     ...
     * @param context an optional context in which to call the callback (callback.apply(context)).
     */
    OEIS.prototype.searchByTermsAndStart = function (terms, start, format, callback) {
        if (!format) {
            format = "html";
        }
        var URL = this.endpoint + "search?" + "language=" + this.language + "&" + "q=" + terms.toString() + "&" + "fmt=" + format + "&" + "start=" + start;
        this.doRequest(URL, format, callback);
    };
    /**
     ...
     * @param context an optional context in which to call the callback (callback.apply(context)).
     */
    OEIS.prototype.searchByQuery = function (queryString, format, callback) {
     ...
     * @param context an optional context in which to call the callback (callback.apply(context)).
     */
    OEIS.prototype.searchByQueryAndStart = function (queryString, start, format, callback) {
        if (!format) {
            format = "html";
        }
        var URL = this.endpoint + "search?" + "language=" + this.language + "&" + "q=" + encodeURIComponent(queryString) + "&" + "fmt=" + format + "&" + "start=" + start;
        this.doRequest(URL, format, callback);
    };
    /**
    ...
    OEIS.prototype.doRequest = function (URL, format, callback) {
        var xhr = this.createCORSRequest("GET", URL, this, format, callback);
        if (!xhr) {
    ...
    OEIS.prototype.createCORSRequest = function (method, url, oeis, format, callback) {
        ...
        xhr.onload = function () {
            ...
            if (this.customCallback) {
                var customCallback = this.customCallback;
                var customContext = this.customContext;
                ...
                if (this.format) {
                    ...
                    if (theFormat.toLowerCase && theFormat.trim) {
                        ...
                        var cleanFormat = theFormat.toLowerCase().trim();
                        if (cleanFormat === "json") {
    ...
    OEIS.prototype.getGraphForSequenceID = function (id, format, callback) {
        if (!format) {
            format = "png";
        }
        var URL = this.endpoint + id + "/graph?png=1";
        this.doRequest(URL, format, callback);
    };
    return OEIS;
}());

export default defaultOEIS = new OEIS("english", false);
